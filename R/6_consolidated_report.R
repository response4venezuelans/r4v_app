...
r4v_consolidated <- function(data, countryname = NULL, org = NULL, totalmodel = "sum")
...
{# The agregation and calculation processed in this file are base on the information 
  ...
  # Filter data and template by Org
  
  
  
  
  if (is.null(org) || (org=="All")) {
    df <- df
  } else {
    df <- df%>%
      filter(Appealing.Organization.Name == org) 
  }
